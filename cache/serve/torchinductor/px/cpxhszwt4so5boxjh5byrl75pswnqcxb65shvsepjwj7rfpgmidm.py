...
# kernel path: /work/projects/polyullm/shihao/data_quality/cache/serve/torchinductor/v7/cv7vxb2g2dmmupbeu3frftzlhcvho2o5boyfpfhpvzkake3gld3p.py
...
#   where => where
# Graph fragment:
#   %copy_ : Tensor "i64[s72][1]cuda:0" = PlaceHolder[target=copy_]
#   %arg3_1 : Tensor "i64[s35][1]cuda:0" = PlaceHolder[target=arg3_1]
#   %where : Tensor "i64[s72][1]cuda:0" = PlaceHolder[target=where]
#   %lt : Tensor "b8[s72][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.lt.Scalar](args = (%arg1_1, 0), kwargs = {})
#   %neg : Tensor "i64[s72][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.neg.default](args = (%arg1_1,), kwargs = {})
#   %clamp_min : Tensor "i64[s72][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.clamp_min.default](args = (%neg, 0), kwargs = {})
#   %index : Tensor "i64[s72][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg3_1, [%clamp_min]), kwargs = {})
#   %where : Tensor "i64[s72][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%lt, %index, %arg1_1), kwargs = {})
#   %copy_ : Tensor "i64[s72][1]cuda:0"[num_users=0] = call_function[target=torch.ops.aten.copy_.default](args = (%arg1_1, %where), kwargs = {})
#   return %where,%buf5
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(0):
            ...
            triton_poi_fused_clamp_copy__index_lt_neg_where_0.run(arg1_1, arg3_1, arg1_1, s35, s72, stream=stream0)
            del arg1_1
            del arg3_1
        return ()
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg2_1 = 2128
    arg3_1 = rand_strided((2128, ), (1, ), device='cuda:0', dtype=torch.int64)
